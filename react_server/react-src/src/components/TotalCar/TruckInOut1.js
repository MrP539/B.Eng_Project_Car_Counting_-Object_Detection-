...
class TruckInOut1 extends Component {
  render() {
    const { TotalsumstruckInWeek, TotalsumstruckOutWeek } = this.props;
    return (
      ...
      <div style={{ backgroundColor: "#ffccbc", color: 'white', padding: '10px', border: '2px solid #ffccbc', fontSize: 'clamp(12px, 2vw, 18px)', textAlign: 'center', borderRadius: '10px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#bf360c', textAlign: 'center' }}>{TotalsumstruckInWeek}</div>
          <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#bf360c', textAlign: 'center' }}>{TotalsumstruckOutWeek}</div>
        </div>
      </div>
    </div>
    );
