...
class CarInOut3 extends Component {
  render() {
    const { TotalsumscarInDay, TotalsumscarOutDay } = this.props; // เพิ่มการดึง props ที่ส่งมาจาก parent component
    return (
      <div style={{ width: '30vw', maxWidth: '100%', flexDirection: 'column', alignItems: 'center', marginLeft: '10px', marginBottom: '10px' }}>
        <div style={{ backgroundColor: "#f0f4c3", color: 'white', padding: '10px', border: '2px solid #f0f4c3', fontSize: 'clamp(12px, 2vw, 18px)', textAlign: 'center', borderRadius: '10px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#1a237e', textAlign: 'center' }}>{TotalsumscarInDay}</div>
            <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#1a237e', textAlign: 'center' }}>{TotalsumscarOutDay}</div>
          </div>
        </div>
      </div>
    );
