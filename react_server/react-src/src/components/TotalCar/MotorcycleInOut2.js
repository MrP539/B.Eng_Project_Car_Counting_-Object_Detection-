...
class MotorcycleInOut2 extends Component {
  render() {
    const { TotalsumsmotorbikeInMonth, TotalsumsmotorbikeOutMonth } = this.props;
    ...
    return (
      ...
      <div style={{ backgroundColor: "#e1bee7", color: 'white', padding: '10px', border: '2px solid #e1bee7', fontSize: 'clamp(12px, 2vw, 18px)', textAlign: 'center', borderRadius: '10px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#4a148c', textAlign: 'center' }}>{TotalsumsmotorbikeInMonth}</div>
          <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#4a148c', textAlign: 'center' }}>{TotalsumsmotorbikeOutMonth}</div>
        </div>
      </div>
    </div>
    );
...
export default MotorcycleInOut2;
