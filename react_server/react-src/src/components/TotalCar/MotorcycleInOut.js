...
class MotorcycleInOut extends Component {
  render() {
    const { Totalsumsmotorbikein, Totalsumsmotorbikeout } = this.props;
      ...
      <div style={{ width: '30vw', maxWidth: '100%', flexDirection: 'column', alignItems: 'center', marginLeft: '10px', marginBottom: '10px' }}>
        <div style={{ backgroundColor: "#e1bee7", color: 'white', padding: '10px', border: '2px solid #e1bee7', fontSize: 'clamp(12px, 2vw, 18px)', textAlign: 'center', borderRadius: '10px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#4a148c', textAlign: 'center' }}>{Totalsumsmotorbikein}</div>
            <div style={{ width: '50%', fontSize: 'clamp(18px, 2.5vw, 20px)', fontWeight: 'bold', padding: '5px', color: '#4a148c', textAlign: 'center' }}>{Totalsumsmotorbikeout}</div>
          </div>
        </div>
      </div>
    );
...
export default MotorcycleInOut;
