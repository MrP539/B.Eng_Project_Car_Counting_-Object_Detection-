...
class ModalConfirmDelete extends Component {
  ...
  handleSubmit(e) {
    ...
    let params = e.target.getAttribute('data-gateID');
    ...
    .then((response) => {
      this.handleClose();
      this.props.onGateDeleted(response.data.result);
  ...
  render() {
    return (
      ...
      >
        ...
        <Modal.Content>
          <p>Are you sure you want to delete <strong>{this.props.gate.name}</strong>?</p>
        </Modal.Content>
        <Modal.Actions>
          <Button onClick={this.handleSubmit} data-gateID={this.props.gate._id} color='red'>Yes</Button>
